...
import { Calendar, Users, MessageSquare, CreditCard, Minus, Plus, Star, Gift, Phone, AlertCircle } from 'lucide-react';
...
function PaytmRedirectForm({ txnToken, orderId, mid }: { txnToken: string; orderId: string; mid: string }) {
    return (
        <div className="rounded-2xl border border-stone-200 bg-white p-10 text-center shadow-sm">
            <div className="h-12 w-12 border-4 border-brand-900 border-t-transparent rounded-full animate-spin mx-auto mb-5" />
            <p className="text-stone-700 font-semibold text-lg">Redirecting to PayTM</p>
            <p className="text-stone-400 text-sm mt-1">Please do not close this window...</p>
            <form id="paytm-form" method="POST"
            ...
            <script dangerouslySetInnerHTML={{ __html: `document.getElementById('paytm-form').submit();` }} />
        </div>
    );
}
        ...
        <div className="flex items-center justify-between rounded-xl border border-stone-200 bg-stone-50/50 px-4 py-3">
            ...
            <div className="flex items-center gap-3">
                <button type="button" onClick={() => onChange(Math.max(min, value - 1))}
                    className="h-8 w-8 rounded-xl border border-stone-200 bg-white flex items-center justify-center text-stone-600 hover:bg-stone-100 hover:border-stone-300 transition-all disabled:opacity-40"
                    disabled={value <= min}>
                    <Minus className="h-3.5 w-3.5" />
                </button>
                <span className="w-5 text-center text-base font-bold text-stone-900">{value}</span>
                <button type="button" onClick={() => onChange(value + 1)}
                    className="h-8 w-8 rounded-xl border border-stone-200 bg-white flex items-center justify-center text-stone-600 hover:bg-stone-100 hover:border-stone-300 transition-all">
...
export default function CheckoutForm({
    ...
    userId, userName, userEmail, userPhone, loyaltyBalance, existingBookingId,
}: Props) {
    ...
    const [paytmData, setPaytmData] = useState<{ txnToken: string; orderId: string; mid: string } | null>(null);

    const today = new Date().toISOString().split('T')[0];
    const subtotal = basePrice * (adults + children * 0.5);
    const pointsDiscount = usePoints && loyaltyBalance >= MIN_REDEEM ? Math.min(loyaltyBalance, subtotal) : 0;
    ...
    const handlePay = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setPaytmData({
                ...
                mid: process.env.NEXT_PUBLIC_PAYTM_MID || 'XVTaon26026633479601',
            });
        } catch (err: any) {
    ...
    if (paytmData) return <PaytmRedirectForm {...paytmData} />;
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-stone-500">
                                    {loyaltyBalance.toLocaleString()} pts available · worth ₹{loyaltyBalance.toLocaleString()}
                                </p>
                            </div>
                        </div>
                    ...
                    {usePoints && pointsDiscount > 0 && (
                        <div className="mt-3 rounded-xl bg-gold-100 border border-gold-200 px-3 py-2.5 flex items-center gap-2">
                            ...
                            <p className="text-sm font-semibold text-gold-800">
                                ...
                                <span className="font-normal text-gold-600 ml-1">({Math.round(pointsDiscount)} points will be redeemed)</span>
                            </p>
                        </div>
                    )}
                </div>
